    ...
    <div class="product_details">
      ...
      <div id="shop_detail">
        ...
        <div class="right_detail lf">
          <!-- 商品名称-->
          <h1 class="title">{{ p1.title }}</h1>
          <!-- 商品全称-->
          <h3 class="subtitle">{{ p1.subtitle }}</h3>
          <!-- 价格部分-->
          <div class="price">
            <div id="pro_price"><b>学员售价：</b><span>￥{{ p1.price }}</span></div>
            <div class="promise">
              <b>服务承诺：</b>
              {{ p1.promise }}
            </div>
          ...
          <p class="spec">
            <s>规格：</s>
            <router-link v-for="(s,i) of others"
                         :key="i" :active-class="p1.lid===s.lid?'active':''"
                         :to="`/details/${s.lid}`">{{
                s.spec
              }}
            </router-link>
...
export default {
  ...
  props: ["lid"],
  created() {
    this.axios.get('http://www.codeboy.com:9999/data/product/details.php', {
      params: {
        lid: this.lid
      }
    })
        .then(res => {
          this.p1 = res.data.product
          this.others = res.data.family.laptopList
        })
  }
}
</script>
