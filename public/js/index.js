...
$('.seek').focus(function(){
    ...
    if($(this).hasClass('clickhover')){
        ...
        $(this).find('images').attr('src','../images/header/header_normal.png');

    }else{
        $(this).addClass('clickhover');
        $(this).find('.seek_content').show();
        $(this).find('images').attr('src','../images/header/header_true.png');
    }
})
$('.seek_content>div').click(function(){
    ...
    $('.seek span').html(text);
    $(this).parent().hide();
    $('.seek').find('images').attr('src','../images/header/header_normal.png');
    $('.seek').blur();
...
$('.aside_nav>p').click(function(){
    if($(this).hasClass('')){
        ...
        var pic=$(this).find('images').attr('src');

        var slicepic=pic.slice(0,-6);
        var newpic=slicepic+'.png';
        $(this).find('images').attr('src',newpic);
        $(this).siblings().each(function(){
            if($(this).hasClass('has')){
                var pic1=$(this).find('images').attr('src');

                var slicepic1=pic1.slice(0,-4);
                var newpic1=slicepic1+'_1.png';
                $(this).find('images').attr('src',newpic1);
